...
<div class="group overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-100 transition hover:shadow-md">
    <div class="relative aspect-[4/3] w-full overflow-hidden bg-gray-100">
        <a href="{{ route($routePrefix.'.properties.show', $property) }}" class="block h-full w-full">
            @if ($cover)
        ...
        @if ($routePrefix === 'agent')
            <form method="POST" action="{{ route('agent.properties.favorite', $property) }}" class="absolute right-3 top-3">
                @csrf
                @method('PATCH')
    ...
    <div class="p-4">
        ...
        <a href="{{ route($routePrefix.'.properties.show', $property) }}" class="mt-0.5 block font-medium text-gray-900 hover:text-blue-600">{{ $property->title }}</a>
        ...
        @if ($showAgent && $property->agent)
            <p class="mt-1 text-xs text-gray-400">Listed by {{ $property->agent->f_name }} {{ $property->agent->l_name }}</p>
        @endif
