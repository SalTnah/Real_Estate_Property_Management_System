...
<x-agent-layout :title="$appointment->title">
    <nav class="text-sm text-gray-500">
        <a href="{{ route('agent.appointments.index') }}" class="text-blue-600 hover:text-blue-700">Appointments</a>
    ...
    <div class="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div>
                        ...
                        <dd class="mt-0.5 font-medium text-gray-900">
                            @if ($appointment->client)
                                <a href="{{ route('agent.clients.show', $appointment->client) }}" class="text-blue-600 hover:text-blue-700">
                    ...
                    <div>
                        ...
                        <dd class="mt-0.5 font-medium text-gray-900">
                            @if ($appointment->property)
                                <a href="{{ route('agent.properties.show', $appointment->property) }}" class="text-blue-600 hover:text-blue-700">
        ...
        <div class="space-y-6">
            @can('update', $appointment)
                <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
                    <a href="{{ route('agent.appointments.edit', $appointment) }}" class="block w-full rounded-lg border border-gray-200 px-4 py-2.5 text-center text-sm font-semibold text-gray-700 hover:bg-gray-50">
                        Edit Appointment
                    </a>
                    @can('delete', $appointment)
                        <form method="POST" action="{{ route('agent.appointments.destroy', $appointment) }}" class="mt-2" onsubmit="return confirm('Cancel this appointment? This cannot be undone.');">
                            @csrf
                            @method('DELETE')
