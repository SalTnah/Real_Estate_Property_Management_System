<x-agent-layout :title="'Edit Property'">
    <div>
    ...
    <form method="POST" action="{{ route('agent.properties.update', $property) }}" enctype="multipart/form-data" class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            @include('properties._photos_panel', ['property' => $property, 'routePrefix' => 'agent'])

            <button type="submit" class="w-full rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">
                Save Property
            </button>
            <a href="{{ route('agent.properties.show', $property) }}" class="block text-center text-sm font-medium text-gray-500 hover:text-gray-700">Cancel</a>
        </div>
    </form>
</x-agent-layout>
