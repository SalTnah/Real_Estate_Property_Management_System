...
<x-agent-layout :title="'Properties'">
    <div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div class="flex shrink-0 items-center gap-3">
            <a href="{{ route('agent.properties.filter') }}" class="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">
                <x-agent.icon name="filter" class="h-4 w-4" />
                Filter &amp; Sort
            </a>
            <a href="{{ route('agent.properties.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">
    ...
    @if ($properties->isEmpty())
        <div class="mt-8 rounded-xl bg-white p-10 text-center shadow-sm ring-1 ring-gray-100">
            ...
            <a href="{{ route('agent.properties.index') }}" class="mt-2 inline-block text-sm font-medium text-blue-600 hover:text-blue-700">Clear filters</a>
        </div>
    @elseif ($view === 'list')
                    ...
                    <tbody>
                        @foreach ($properties as $property)
                            <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                                <td class="px-5 py-3">
                                    <a href="{{ route('agent.properties.show', $property) }}" class="font-medium text-gray-900 hover:text-blue-600">{{ $property->title }}</a>
    ...
    @else
        <div class="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($properties as $property)
                @include('properties._card', ['property' => $property, 'routePrefix' => 'agent'])
            @endforeach
        </div>
    @endif
