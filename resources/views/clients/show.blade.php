...
<x-agent-layout :title="$client->f_name.' '.$client->l_name">
    <nav class="text-sm text-gray-500">
        <a href="{{ route('clients.index') }}" class="text-blue-600 hover:text-blue-700">Clients</a>
    ...
    <div class="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6">
            ...
            @can('update', $client)
                <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
                    <a href="{{ route('clients.edit', $client) }}" class="block w-full rounded-lg border border-gray-200 px-4 py-2.5 text-center text-sm font-semibold text-gray-700 hover:bg-gray-50">
                        Edit Client
                    </a>
                    @can('delete', $client)
                        <form method="POST" action="{{ route('clients.destroy', $client) }}" class="mt-2" onsubmit="return confirm('Delete this client? This cannot be undone.');">
                            @csrf
                            @method('DELETE')
